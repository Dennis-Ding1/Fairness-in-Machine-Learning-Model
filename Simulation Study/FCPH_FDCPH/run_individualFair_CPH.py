...
sys.stdout = open(os.path.join(output_dir, "individual_fair_cox_ph_output_scale.txt"), "w")
...
model_path = os.path.join(script_dir, "trained-models", "Individual_Fair_CoxPH.pth")
if os.path.exists(model_path):
    coxPH_model.load_state_dict(torch.load(model_path))
    print(f"Loaded pretrained model from {model_path}", flush=True)
else:
    print("No pretrained model found — training from scratch.", flush=True)
    #%% training cox ph model    
    
    for epoch in range(num_epochs):   
        # print(f"\nEpoch {epoch+1}/{num_epochs}")
        # for batch in tqdm(
        #     range(0, np.int64(np.floor(len(data_X_train)/mini_batch))*mini_batch, mini_batch),
        ...
        for batch in range(0,np.int64(np.floor(len(data_X_train)/mini_batch))*mini_batch,mini_batch):       
            ...
            total_loss.backward()
            optimizer.step()
        print ('Epoch [{}/{}], Loss: {:.4f}'.format(epoch+1, num_epochs, total_loss.item()), flush=True)
...
for t in range(len(eval_time)):
    ...
    for i in range(len(data_X_test)):
        ...
        index = np.argmin(np.abs(time_point - eval_time[t]))
...
plt.grid(True)
plt.savefig(os.path.join(output_dir, 'individualFair_auc.png'),dpi = 600)
...
#%% save the model
model_dir = os.path.join(script_dir, "trained-models")

torch.save(coxPH_model.state_dict(), os.path.join(model_dir, "Individual_Fair_CoxPH.pth"))
