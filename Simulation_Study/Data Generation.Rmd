...
ggplot(dat, aes(x = Y, fill = factor(Delta))) +
  geom_histogram(alpha = 0.5, bins = 40, position = "identity") +
  labs(title = "SIM_I Obsereved Time by censoring status (0 censored, 1 uncensored) ",
       x = "Observed Time",
       fill = "Censoring status") +
  theme_minimal()
...
ggplot(dat, aes(x = Y, fill = factor(Delta))) +
  geom_histogram(alpha = 0.5, bins = 40, position = "identity") +
  labs(title = "SIM_II Obsereved Time by censoring status (0 censored, 1 uncensored) ",
       x = "Observed Time",
       fill = "Censoring status") +
  theme_minimal()
...
summary_all %>%
  kable("latex", booktabs = TRUE, caption = "Summary Statistics for Two Simulated Datasets") %>%
...
censor_table <- function(df, name){
  df %>%
    ...
    summarise(
      Censor_A0 = mean(censored[A == 0], na.rm=TRUE) * 100,
      Censor_A1 = mean(censored[A == 1], na.rm=TRUE) * 100,
      Censor_Overall = mean(censored, na.rm=TRUE) * 100
    ) %>%
        mutate(
      Dataset = name,
      across(starts_with("Censor_"), ~ sprintf("%.1f%%", .x))  # 转成百分比格式
    ) %>%
    select(Dataset, everything())
}

cen_I  <- censor_table(data_I, "SIM_I")
cen_II <- censor_table(data_II, "SIM_II")
...
ggplot(flc, aes(x = time, fill = factor(status))) +
  geom_histogram(alpha = 0.5, bins = 40, position = "identity") +
  labs(title = "FLChain Obsereved Time by censoring status (0 censored, 1 uncensored) ",
       x = "Observed Time",
       fill = "Censoring status") +
  theme_minimal()
